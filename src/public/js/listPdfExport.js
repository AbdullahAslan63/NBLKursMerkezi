 ...
 * Öğrenci ve Öğretmen listelerini yazdırma dostu PDF olarak dışa aktarır.
 */
...
export async function exportListPdf({ title, filename, tbodyId, colIndices, triggerBtn }) {
  // Buton durumunu "Hazırlanıyor…" yap
  let originalText = '';
  if (triggerBtn) {
  ...
  try {
    // jsPDF ve autoTable'ın yüklenmesini bekle
 ...
 * @param {object} options
 * @param {HTMLButtonElement} [options.triggerBtn]
 */
  ...
  try {
    ...
    const rows = slots.map(slotKey => {
      ...
      daysConfig.forEach(day => {
        ...
        if (matches.length > 0) {
          // Aynı saat dilimindeki etütleri ders ve öğretmen adlarıyla alt alta yaz
    ...
    doc.autoTable({
      ...
      columnStyles: {
        0: { fontStyle: 'bold', halign: 'center', fillColor: [240, 240, 245], cellWidth: 25 },
      },
      headStyles: {
        fillColor: [41, 41, 71],
        textColor: [255, 255, 255],
 ...
 * İlk çağrıda yüklenir, sonraki çağrılarda anında döner.
 */
